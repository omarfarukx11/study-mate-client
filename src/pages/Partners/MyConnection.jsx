...
const MyConnection = () => {
  ...
  return (
    <div className="min-h-[calc(100vh-120px)] bg-secondary mx-auto rounded-lg text-neutral-content">
      ...
      <div className="lg:h-[200px] lg:py-0 py-10 bg-secondary flex items-center justify-center flex-col rounded-sm">
        ...
        <p className="text-center lg:py-5 py-2 text-gray-500 text-sm">
          ...
          Delete requests that are no longer relevant. Keep your study partners
          list up-to-date and organized.
        </p>
      </div>

      {/* Logic: If no requests for THIS user, show the text. Otherwise show the table */}
      {filteredRequests.length === 0 ? (
        <div className="px-4">
          <p className="bg-primary lg:text-5xl text-xl text-center font-semibold my-10 text-white p-10 rounded-sm leading-tight">
          ...
          <table className="w-full">
            ...
            <tbody className="text-center">
              {filteredRequests.map((data, index) => (
                <MyRequest key={data._id} index={index} data={data} />
              ))}
            </tbody>
          </table>
        </div>
